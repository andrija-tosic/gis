...
config.read("import_emission.conf")
...
# Create table with additional fields for emission data
...
db.execute(f"SELECT create_hypertable('{table_name}', 'timestamp');")
...
insert_query = f"""
    INSERT INTO {table_name} (
        osm_id, osm_obj, timestamp, obj_type, speed, angle, lane_or_edge, way,
        CO2, CO, HC, NOx, PMx, fuel, electricity, noise
    )
...
with open(file_path, 'r') as fin:
    for line in fin:
        total_lines += 1
        data_count += line.count('<vehicle id')
...
for (event, timestep) in context:
    ...
    for obj in timestep:
        ...
        CO2 = float(obj.attrib.get('CO2', 0.0))
        CO = float(obj.attrib.get('CO', 0.0))
        HC = float(obj.attrib.get('HC', 0.0))
        NOx = float(obj.attrib.get('NOx', 0.0))
        PMx = float(obj.attrib.get('PMx', 0.0))
        fuel = float(obj.attrib.get('fuel', 0.0))
        electricity = float(obj.attrib.get('electricity', 0.0))
        noise = float(obj.attrib.get('noise', 0.0))
        ...
        db.execute(insert_query, (
            osm_id, osm_obj, timestamp, obj_type, obj_speed, obj_angle, lane_or_edge, obj_lon, obj_lat,
            CO2, CO, HC, NOx, PMx, fuel, electricity, noise
        ))
